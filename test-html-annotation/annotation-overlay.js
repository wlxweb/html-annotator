import { createElement, icons } from './node_modules/lucide/dist/esm/lucide.mjs';
...
function refreshLucide() {
  ...
  nodes.forEach((node) => {
    ...
    const svg = createElement(iconNode, {
      class: ['lucide', `lucide-${name}`, node.getAttribute('class') || ''].filter(Boolean).join(' '),
      width: node.getAttribute('data-size') || 16,
      height: node.getAttribute('data-size') || 16,
      stroke: 'currentColor'
    });
